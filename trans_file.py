...
# 翻译文献到新的pdf以及word中
path_pdf_original = root + r"/trans/input_file/JP.pdf"  # 这里改pdf的名字
file_name = path_pdf_original.split("/")[-1]
print('当前翻译的pdf名字', file_name)
t0 = time.time()
cur_pdf = fitz.open(path_pdf_original)  # 待翻译的pdf
...
try:
	for cur_page in cur_pdf:
		...
		for img in img_list:  # 获取当前页面的图像列表
			...
			pix_temp.writeImage(os.path.join(root, 'trans', 'output_file', new_name))
		...
		for num in range(len(blks)):  # loop through the blocks
			...
			if num < len(blks) - 1:
				...
				if (abs(blks[num + 1][1] - blks[num][3]) <= 1.0 and abs(blks[num + 1][1] - blks[num][3]) >= 0):
					# 当前块在参考文献之后
					if reference_flag == 1:
						trans_pragraph = blks[num][4].replace("/n", " ")
					...
					if flag == 1:
						...
						if begin[2] > end[2]:  # 如果起始点的右下角x坐标小于结束点的右下角x坐标
							new_page.insertTextbox(fitz.Rect(end[0], begin[1], begin[2], end[3]), res,
							                       fontname = "song", fontfile = os.path.join(root,
							                                                                  'trans/static/fonts/SimSun.ttf'),
